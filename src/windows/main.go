...
func StaticallyInitialize(program []string) {
	...
	fmt.Println("FIRST ASSIGNMENT RUN \nSTATUS: COMPLETE\n")
...
func StartExecution(program []string) {
	for i := 0; i < len(methodNames); i++ {
		...
		fmt.Println("VAR SYNTAX REPLACEMENT IN METHOD [",methodNames[i],"] \nSTATUS: COMPLETE\n")
	}
...
func callFunctionMAIN() (string){
	fmt.Println("EXECUTION ENVIRONMENT SET\nCODE EXECUTION STARTING\n\n")
...
func (caller ScopeNode) takeInput (line string) {
	...
	if len(parts) < 2 || len(parts) > 2 {
		fmt.Println(""+INSUFFICIENT_ARGS_FOR_INPUT_ERR,"\n")
		os.Exit(0)
	}
...
func createStringRep (line string,TYPE string) (string){
	switch TYPE {
	case STRING:
		return ""+string([]rune(line)[1:len(line)-1])
	case BOOLEAN:
	...
	func(arg1 Data,arg2 Data)(string,bool){
		if arg1.Type == NUMBER {
			...
			if arg2.Type == NUMBER {
				...
				fmt.Println(" illegal conversion of charge to number")
				os.Exit(0)
			...
			if arg2.Type == DOUBLE {
				...
				fmt.Println(" illegal conversion of charge to Double")
				os.Exit(0)
